...
# Canonical public Egress installer entrypoint. The implementation is split
# under egress/scripts/install.
...
egress_entrypoint_local_source_dir() {
  ...
  entrypoint_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd) ||
    return 1
  [ -f "$entrypoint_dir/scripts/install/main.sh" ] || return 1
  printf '%s/scripts/install' "$entrypoint_dir"
}
