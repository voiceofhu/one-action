...
resolve_latest_version() {
  ...
  version=$(jq -er '
    [
      ...
      | .tag_name
      | select(startswith("one-browser-egress-v"))
    ][0]
    | sub("^one-browser-egress-v"; "")
  ' "$releases") || {
    rm -f "$releases"
...
download_native_binary() {
  ...
  release_base="https://github.com/voiceofhu/one-action/releases/download/one-browser-egress-v$RESOLVED_VERSION"
