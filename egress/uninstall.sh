...
# Canonical public Egress uninstaller entrypoint. The implementation is split
# under egress/scripts/uninstall.

set +x
set -Eeuo pipefail

readonly ONE_BROWSER_EGRESS_DEFAULT_SCRIPT_BASE_URL='https://raw.githubusercontent.com/voiceofhu/one-action'
...
egress_entrypoint_local_source_dir() {
  ...
  entrypoint_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd) ||
    return 1
  [ -f "$entrypoint_dir/scripts/uninstall/main.sh" ] || return 1
  printf '%s/scripts/uninstall' "$entrypoint_dir"
}
