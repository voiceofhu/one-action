...
uninstall_path_kind() {
	...
	"$MANIFEST_INSTALLER_PATH"|"$MANIFEST_BINARY_PATH"|"$MANIFEST_PREVIOUS_BINARY_PATH_FIXED"|"$MANIFEST_ENV_PATH"|"$MANIFEST_COMPOSE_PATH"|"$MANIFEST_RECORD_PATH"|"$MANIFEST_UNIT_PATH")
		printf '%s\n' file
		;;
	*) return 1 ;;
	esac
