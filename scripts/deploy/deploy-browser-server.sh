...
PUBLIC_URL=${PUBLIC_URL:-https://browser.aicbe.com}
...
[[ "$DOCKER_IMAGE" =~ ^ghcr\.io/voiceofhu/one-browser-backend:(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)@sha256:[0-9a-f]{64}$ ]] || {
  printf '%s\n' 'DOCKER_IMAGE must be the fixed One Browser Server version pinned to an OCI index digest' >&2
  exit 1
}
...
image=$2
public_url=$3
container_name=one-browser-server
service_name=server
...
[[ "$configured_images" == "$image" ]] || {
  printf '%s\n' 'Compose did not resolve exactly the requested immutable One Browser Server image' >&2
  exit 1
}
...
rollback() {
  ...
  printf '%s\n' 'One Browser Server deployment failed; attempting to restore the previous container' >&2
  if [[ "$previous_image" =~ ^ghcr\.io/voiceofhu/one-browser-backend:.*@sha256:[0-9a-f]{64}$ ]] \
...
docker cp "$extract_container:/opt/one-browser/web-dist/." "web/$release_web/"
...
for attempt in $(seq 1 30); do
  ...
  published_port="$(docker port "$container_name" 27514/tcp 2>/dev/null | sed -n '1s/.*://p' || true)"
  if [[ -n "$published_port" ]] \
    && curl --fail --silent --show-error --connect-timeout 2 --max-time 5 \
      "http://127.0.0.1:$published_port/healthz" >/dev/null \
    && curl --fail --silent --show-error --connect-timeout 2 --max-time 5 \
      "http://127.0.0.1:$published_port/" >/dev/null; then
    printf '%s\n' 'One Browser Server is healthy and serving Web at /'
    break
  fi
  if [[ "$attempt" == 30 ]]; then
    docker logs --tail 120 "$container_name" >&2 || true
...
for attempt in $(seq 1 30); do
  if curl --fail --silent --show-error --connect-timeout 3 --max-time 10 \
    ...
    public_ready=true
    printf 'One Browser Server public endpoint is ready: %s\n' "$public_url"
    break
  fi
  if [[ "$attempt" == 30 ]]; then
