...
ssh "$SSH_HOST" "test -L '$REMOTE_DIR/web/current' && test -f '$REMOTE_DIR/web/current/index.html'" || {
  echo 'Deploy Browser Server with the persistent Web mount before publishing Web separately.' >&2
  exit 1
}
...
# Preserve hashed assets referenced by pages already open in a browser.
