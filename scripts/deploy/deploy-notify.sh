...
[[ "$DOCKER_IMAGE" =~ ^ghcr\.io/voiceofhu/one-notify:(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)@sha256:[0-9a-f]{64}$ ]] || {
  printf '%s\n' 'DOCKER_IMAGE must be the fixed One Notify version pinned to an OCI index digest' >&2
  exit 1
}
...
[[ "$configured_images" == "$image" ]] || {
  printf '%s\n' 'Compose did not resolve exactly the requested immutable One Notify image' >&2
  exit 1
}
...
rollback() {
  ...
  printf '%s\n' 'One Notify deployment failed; attempting to restore the previous container' >&2
  if [[ "$previous_image" =~ ^ghcr\.io/voiceofhu/one-notify:.*@sha256:[0-9a-f]{64}$ ]] \
...
docker cp "$extract_container:/opt/one-notify/web-dist/." "web/$release_web/"
...
[[ "$running_image" == "$image" ]] || {
  printf '%s\n' 'Running container does not use the requested immutable One Notify image' >&2
  false
}
...
for attempt in $(seq 1 30); do
  if curl --fail --silent --show-error --connect-timeout 3 --max-time 10 \
    ...
    public_ready=true
    printf 'One Notify public endpoint is ready: %s\n' "$public_url"
    break
  fi
  if [[ "$attempt" == 30 ]]; then
