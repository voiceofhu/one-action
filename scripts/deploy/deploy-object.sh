...
PUBLIC_URL=${PUBLIC_URL:-https://object.aicbe.com}
...
[[ "$DOCKER_IMAGE" =~ ^ghcr\.io/voiceofhu/one-object:(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)@sha256:[0-9a-f]{64}$ ]] || {
  printf '%s\n' 'DOCKER_IMAGE must be the fixed One Object version pinned to an OCI index digest' >&2
  exit 1
}
...
[[ "$configured_images" == "$image" ]] || {
  printf '%s\n' 'Compose did not resolve exactly the requested immutable One Object image' >&2
  exit 1
}
...
rollback() {
  ...
  printf '%s\n' 'One Object deployment failed; attempting to restore the previous container' >&2
  if [[ "$previous_image" =~ ^ghcr\.io/voiceofhu/one-object:.*@sha256:[0-9a-f]{64}$ ]] \
...
docker cp "$extract_container:/opt/one-object/web-dist/." "web/$release_web/"
...
published_port="$(docker port "$container_name" 27525/tcp 2>/dev/null | sed -n '1s/.*://p' || true)"
[[ -n "$published_port" ]] \
...
[[ "$running_image" == "$image" ]] || {
  printf '%s\n' 'Running container does not use the requested immutable One Object image' >&2
  false
}
...
for attempt in $(seq 1 30); do
  if curl --fail --silent --show-error --connect-timeout 3 --max-time 10 \
    ...
    public_ready=true
    printf 'One Object public endpoint is ready: %s\n' "$public_url"
    break
  fi
  if [[ "$attempt" == 30 ]]; then
