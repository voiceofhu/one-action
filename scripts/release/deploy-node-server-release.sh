...
CONFIRM_DISPATCH="dispatch:node-server.yml:$action_head" \
CONFIRM_MUTATION="mutate:node-server.yml:$action_head" \
DRY_RUN=false \
...
bash "$PROJECT_ROOT/scripts/github/dispatch-workflow.sh" node-server.yml \
  ...
  'publish=true' \
  'deploy=true'
printf 'Dispatched One Node Server image publication and deployment at Action %s.\n' \
  "$action_head"
