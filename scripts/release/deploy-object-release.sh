...
: "${ONE_OBJECT_BACKEND_DIR:?ONE_OBJECT_BACKEND_DIR is required}"
: "${ONE_OBJECT_WEB_DIR:?ONE_OBJECT_WEB_DIR is required}"
: "${ONE_OBJECT_BACKEND_REPOSITORY:?ONE_OBJECT_BACKEND_REPOSITORY is required}"
: "${ONE_OBJECT_WEB_REPOSITORY:?ONE_OBJECT_WEB_REPOSITORY is required}"
...
backend_state="$(validate_source_repository \
  Backend "$ONE_OBJECT_BACKEND_DIR" "$ONE_OBJECT_BACKEND_REPOSITORY" Cargo.toml)"
web_state="$(validate_source_repository \
  Web "$ONE_OBJECT_WEB_DIR" "$ONE_OBJECT_WEB_REPOSITORY" package.json)"
...
printf '%s\n' \
  'One Object release plan:' \
  "  version:          $VERSION" \
  "  backend:          $ONE_OBJECT_BACKEND_REPOSITORY@$backend_head" \
  "  web:              $ONE_OBJECT_WEB_REPOSITORY@$web_head" \
...
preflight_release_repository Backend "$ONE_OBJECT_BACKEND_DIR" "$backend_branch"
preflight_release_repository Web "$ONE_OBJECT_WEB_DIR" "$web_branch"

(
  cd "$ONE_OBJECT_BACKEND_DIR"
  cargo fmt --all -- --check
)
...
[[ "$(git -C "$ONE_OBJECT_BACKEND_DIR" rev-parse HEAD)" == "$backend_head" \
  && -z "$(git -C "$ONE_OBJECT_BACKEND_DIR" status --porcelain --untracked-files=all)" ]] || {
  printf '%s\n' 'Backend source changed during local validation.' >&2
  exit 1
}
[[ "$(git -C "$ONE_OBJECT_WEB_DIR" rev-parse HEAD)" == "$web_head" \
  && -z "$(git -C "$ONE_OBJECT_WEB_DIR" status --porcelain --untracked-files=all)" ]] || {
  printf '%s\n' 'Web source changed during local validation.' >&2
  exit 1
}

update_backend_version() {
  local manifest="$ONE_OBJECT_BACKEND_DIR/Cargo.toml"
  local lockfile="$ONE_OBJECT_BACKEND_DIR/Cargo.lock"
  local temp
  ...
  awk -v version="$VERSION" '
    ...
    in_package && /^name = "one-object-backend"[[:space:]]*$/ { is_target=1; print; next }
...
update_web_version() {
  VERSION="$VERSION" node - <<'NODE' "$ONE_OBJECT_WEB_DIR/package.json"
const fs = require('node:fs');
const file = process.argv[2];
const document = JSON.parse(fs.readFileSync(file, 'utf8'));
if (document.name !== 'one-object-web' || typeof document.version !== 'string') {
  throw new Error('package.json does not describe one-object-web');
}
...
update_backend_version
update_web_version
verify_changed_paths Backend "$ONE_OBJECT_BACKEND_DIR" 'Cargo.lock Cargo.toml'
verify_changed_paths Web "$ONE_OBJECT_WEB_DIR" 'package.json'

git -C "$ONE_OBJECT_BACKEND_DIR" add -- Cargo.toml Cargo.lock
if ! git -C "$ONE_OBJECT_BACKEND_DIR" diff --cached --quiet; then
  git -C "$ONE_OBJECT_BACKEND_DIR" commit -m \
    "chore: bump one-object-backend version to $VERSION"
fi

git -C "$ONE_OBJECT_WEB_DIR" add -- package.json
if ! git -C "$ONE_OBJECT_WEB_DIR" diff --cached --quiet; then
  git -C "$ONE_OBJECT_WEB_DIR" commit -m \
    "chore: bump one-object-web version to $VERSION"
fi

if ! git -C "$ONE_OBJECT_BACKEND_DIR" push origin \
  "HEAD:refs/heads/$backend_branch"; then
  printf '%s\n' 'Backend version commit push failed; dispatch was not attempted.' >&2
  exit 1
fi
if ! git -C "$ONE_OBJECT_WEB_DIR" push origin \
...
backend_release_sha="$(git -C "$ONE_OBJECT_BACKEND_DIR" rev-parse HEAD)"
web_release_sha="$(git -C "$ONE_OBJECT_WEB_DIR" rev-parse HEAD)"
printf '%s\n' \
  "Published Backend source: $ONE_OBJECT_BACKEND_REPOSITORY@$backend_release_sha" \
  "Published Web source: $ONE_OBJECT_WEB_REPOSITORY@$web_release_sha"

CONFIRM_DISPATCH="dispatch:object-server.yml:$action_head" \
CONFIRM_MUTATION="mutate:object-server.yml:$action_head" \
DRY_RUN=false \
ACTION_REPOSITORY="$ACTION_REPOSITORY" \
ACTION_REF=main \
GH_TOKEN="$github_token" \
bash "$PROJECT_ROOT/scripts/github/dispatch-workflow.sh" object-server.yml \
  "backend_repository=$ONE_OBJECT_BACKEND_REPOSITORY" \
  "backend_ref=$backend_release_sha" \
  "web_repository=$ONE_OBJECT_WEB_REPOSITORY" \
  "web_ref=$web_release_sha" \
  "version=$VERSION" \
  'publish=true'
printf 'Dispatched One Object image publication and server deployment at Action %s.\n' \
  "$action_head"
