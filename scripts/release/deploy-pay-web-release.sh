...
web_state="$(validate_source_repository Web "$ONE_PAY_WEB_DIR" "$ONE_PAY_WEB_REPOSITORY" package.json)"
web_branch=${web_state%%|*}
web_head=${web_state#*|}
action_head="$(git -C "$PROJECT_ROOT" rev-parse --verify HEAD)"
printf 'One Pay Web: %s at %s, version %s\n' "$ONE_PAY_WEB_REPOSITORY" "$web_head" "$VERSION"
...
make --no-print-directory -C "$PROJECT_ROOT" validate-pay-web
git -C "$ONE_PAY_WEB_DIR" fetch --no-tags origin "refs/heads/$web_branch:refs/remotes/origin/$web_branch"
[[ "$web_head" == "$(git -C "$ONE_PAY_WEB_DIR" rev-parse "origin/$web_branch")" ]]
pnpm --dir "$ONE_PAY_WEB_DIR" install --frozen-lockfile
pnpm --dir "$ONE_PAY_WEB_DIR" lint
pnpm --dir "$ONE_PAY_WEB_DIR" build
[[ "$(validate_source_repository Web "$ONE_PAY_WEB_DIR" "$ONE_PAY_WEB_REPOSITORY" package.json)" == "$web_state" ]]
[[ "$(validate_action_repository)" == "$action_head" ]]
CONFIRM_DISPATCH="dispatch:pay-web.yml:$action_head" \
CONFIRM_MUTATION="mutate:pay-web.yml:$action_head" \
DRY_RUN=false ACTION_REPOSITORY="$ACTION_REPOSITORY" ACTION_REF=main GH_TOKEN="$github_token" \
bash "$PROJECT_ROOT/scripts/github/dispatch-workflow.sh" pay-web.yml \
  "web_repository=$ONE_PAY_WEB_REPOSITORY" "web_ref=$web_head" \
