...
web_state="$(validate_source_repository Web "$ONE_USER_WEB_DIR" "$ONE_USER_WEB_REPOSITORY" package.json)"
web_branch=${web_state%%|*}
web_head=${web_state#*|}
action_head="$(git -C "$PROJECT_ROOT" rev-parse --verify HEAD)"
printf 'One User Web: %s at %s, version %s\n' "$ONE_USER_WEB_REPOSITORY" "$web_head" "$VERSION"
...
make --no-print-directory -C "$PROJECT_ROOT" validate-user-web
git -C "$ONE_USER_WEB_DIR" fetch --no-tags origin "refs/heads/$web_branch:refs/remotes/origin/$web_branch"
[[ "$web_head" == "$(git -C "$ONE_USER_WEB_DIR" rev-parse "origin/$web_branch")" ]]
...
[[ "$(validate_source_repository Web "$ONE_USER_WEB_DIR" "$ONE_USER_WEB_REPOSITORY" package.json)" == "$web_state" ]]
[[ "$(validate_action_repository)" == "$action_head" ]]
CONFIRM_DISPATCH="dispatch:user-web.yml:$action_head" \
CONFIRM_MUTATION="mutate:user-web.yml:$action_head" \
DRY_RUN=false ACTION_REPOSITORY="$ACTION_REPOSITORY" ACTION_REF=main GH_TOKEN="$github_token" \
bash "$PROJECT_ROOT/scripts/github/dispatch-workflow.sh" user-web.yml \
  "web_repository=$ONE_USER_WEB_REPOSITORY" "web_ref=$web_head" \
