...
source=(Path(__file__).resolve().parents[1] / 'scripts/deploy/deploy-node-web.sh').read_text()
...
with tempfile.TemporaryDirectory() as tmp:
 ...
 for fail in ['0','1']:
  ...
  result=subprocess.run(['bash','-s','--',str(remote),sha,digest,'https://node.example.com'],input=body,text=True,capture_output=True,env=dict(os.environ,PATH=str(binary)+':'+os.environ['PATH'],FAIL_HEALTH=fail,TEST_WEB_SHA=sha))
