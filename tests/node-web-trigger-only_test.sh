...
printf '%s\n' 'module example.invalid/one-node-server' 'go 1.25' >"$ONE_NODE_SERVER_DIR/Cargo.toml"
git -C "$ONE_NODE_SERVER_DIR" add Cargo.toml
...
cp "$PROJECT_ROOT/scripts/release/deploy-node-web-release.sh" \
  "$action_dir/scripts/release/deploy-node-web-release.sh"
...
git -C "$action_dir" add scripts/release/deploy-node-web-release.sh scripts/github/dispatch-workflow.sh
...
ONE_NODE_SERVER_DIR="$test_dir/no-server-checkout" bash "$action_dir/scripts/release/deploy-node-web-release.sh" >"$test_dir/output"
...
require_gate "make --no-print-directory -C $action_dir validate-node-web"
require_gate "pnpm --dir $ONE_NODE_WEB_DIR install --frozen-lockfile"
require_gate "pnpm --dir $ONE_NODE_WEB_DIR lint"
require_gate "pnpm --dir $ONE_NODE_WEB_DIR build"

printf '%s\n' 'One Node Web dispatched without a Server checkout or Server build.'
