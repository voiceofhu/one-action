...
cat >"$ONE_NOTIFY_BACKEND_DIR/Cargo.toml" <<'EOF'
[package]
name = "one-notify-backend"
version = "0.1.0"
EOF
cat >"$ONE_NOTIFY_BACKEND_DIR/Cargo.lock" <<'EOF'
version = 4
...
git -C "$ONE_NOTIFY_WEB_DIR" remote add origin \
  https://github.com/voiceofhu/one-notify-web.git
git -C "$ONE_NOTIFY_WEB_DIR" config \
...
cp "$PROJECT_ROOT/scripts/release/deploy-notify-release.sh" \
  "$action_dir/scripts/release/deploy-notify-release.sh"
...
git -C "$action_dir" add scripts/release/deploy-notify-release.sh scripts/github/dispatch-workflow.sh
...
GITHUB_TOKEN=runtime-token-must-not-reach-gates \
  bash "$action_dir/scripts/release/deploy-notify-release.sh" >"$test_dir/output"

grep -Fq 'version = "26.815.1234"' "$ONE_NOTIFY_BACKEND_DIR/Cargo.toml"
grep -Fq 'version = "26.815.1234"' "$ONE_NOTIFY_BACKEND_DIR/Cargo.lock"
node -e '
...
if git --git-dir="$backend_bare" for-each-ref --format='%(refname)' refs/tags | grep -q . ||
  ...
  printf '%s\n' 'One Notify release created a forbidden repository tag.' >&2
  exit 1
fi
...
require_gate "make --no-print-directory -C $action_dir validate-notify"
require_gate 'cargo fmt --all -- --check'
require_gate "make --no-print-directory -C $ONE_NOTIFY_BACKEND_DIR test"
...
if [ -s "$FAKE_CURL_LOG" ]; then
  printf '%s\n' 'Local One Notify release unexpectedly called the GitHub API.' >&2
  exit 1
fi

[[ -z "${GH_TOKEN:-}" ]]
printf '%s\n' 'One Notify version commits dispatched exact SHAs without repository tags.'
