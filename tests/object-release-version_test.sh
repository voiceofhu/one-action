...
cat >"$ONE_OBJECT_BACKEND_DIR/Cargo.toml" <<'EOF'
[package]
name = "one-object-backend"
version = "0.1.0"
EOF
cat >"$ONE_OBJECT_BACKEND_DIR/Cargo.lock" <<'EOF'
version = 4
...
git -C "$ONE_OBJECT_WEB_DIR" remote add origin \
  https://github.com/voiceofhu/one-object-web.git
git -C "$ONE_OBJECT_WEB_DIR" config \
...
cp "$PROJECT_ROOT/scripts/release/deploy-object-release.sh" \
  "$action_dir/scripts/release/deploy-object-release.sh"
...
git -C "$action_dir" add scripts/release/deploy-object-release.sh scripts/github/dispatch-workflow.sh
...
GITHUB_TOKEN=runtime-token-must-not-reach-gates \
  bash "$action_dir/scripts/release/deploy-object-release.sh" >"$test_dir/output"

grep -Fq 'version = "26.815.1234"' "$ONE_OBJECT_BACKEND_DIR/Cargo.toml"
grep -Fq 'version = "26.815.1234"' "$ONE_OBJECT_BACKEND_DIR/Cargo.lock"
node -e '
...
if git --git-dir="$backend_bare" for-each-ref --format='%(refname)' refs/tags | grep -q . ||
  ...
  printf '%s\n' 'One Object release created a forbidden repository tag.' >&2
  exit 1
fi
...
require_gate "make --no-print-directory -C $action_dir validate-object"
require_gate 'cargo fmt --all -- --check'
require_gate "make --no-print-directory -C $ONE_OBJECT_BACKEND_DIR test"
...
if [ -s "$FAKE_CURL_LOG" ]; then
  printf '%s\n' 'Local One Object release unexpectedly called the GitHub API.' >&2
  exit 1
fi

[[ -z "${GH_TOKEN:-}" ]]
printf '%s\n' 'One Object version commits dispatched exact SHAs without repository tags.'
