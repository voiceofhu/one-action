...
ruby -ryaml -e '
  root = ARGV.fetch(0)
  server = YAML.load_file("#{root}/.github/workflows/user-server.yml")
  web = YAML.load_file("#{root}/.github/workflows/user-web.yml")
  ...
  [server, web].each do |workflow|
    ...
    abort "environment" unless deploy.fetch("environment").fetch("name") == "one-user-prod"
    abort "deployment race" unless deploy.fetch("concurrency").fetch("group") == "one-user-prod-deploy"
  ...
  abort "Web must only checkout Web source" unless repos == ["voiceofhu/one-user-web"]
  compose = YAML.load_file("#{root}/../one-user/backend/deploy/docker/docker-compose.yml")
  service = compose.fetch("services").fetch("user")
  abort "persistent web mount" unless service.fetch("volumes").include?("./web:/opt/one-user/web:ro")
  abort "dynamic current path" unless service.fetch("environment").fetch("WEB_DIST_DIR") == "/opt/one-user/web/current"
  release = File.read("#{root}/scripts/release/deploy-user-web-release.sh")
  abort "Web release depends on Server" if release.include?("ONE_USER_BACKEND") || release.include?("cargo ")
  ...
  server_deploy = File.read("#{root}/scripts/deploy/deploy-user.sh")
  ...
  deploy = File.read("#{root}/scripts/deploy/deploy-user-web.sh")
...
' "$PROJECT_ROOT"
printf '%s\n' 'User Server/Web workflow contracts passed.'
