...
printf '%s\n' '[package]' 'name = "one-user-backend"' 'version = "0.1.0"' >"$ONE_USER_BACKEND_DIR/Cargo.toml"
git -C "$ONE_USER_BACKEND_DIR" add Cargo.toml
git -C "$ONE_USER_BACKEND_DIR" commit -qm initial
git -C "$ONE_USER_BACKEND_DIR" remote add origin https://github.com/voiceofhu/one-user-backend.git
git -C "$ONE_USER_BACKEND_DIR" config url."file://$backend_bare".insteadOf \
...
web_sha="$(git -C "$ONE_USER_WEB_DIR" rev-parse HEAD)"
git -C "$ONE_USER_WEB_DIR" remote add origin https://github.com/voiceofhu/one-user-web.git
git -C "$ONE_USER_WEB_DIR" config url."file://$web_bare".insteadOf \
...
cp "$PROJECT_ROOT/scripts/release/deploy-user-web-release.sh" \
  "$action_dir/scripts/release/deploy-user-web-release.sh"
...
git -C "$action_dir" add scripts/release/deploy-user-web-release.sh scripts/github/dispatch-workflow.sh
...
ONE_USER_BACKEND_DIR="$test_dir/no-server-checkout" bash "$action_dir/scripts/release/deploy-user-web-release.sh" >"$test_dir/output"
...
require_gate "make --no-print-directory -C $action_dir validate-user-web"
...
printf '%s\n' 'One User Web dispatched without a Server checkout or Server build.'
