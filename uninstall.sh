...
# Legacy compatibility entrypoint. New commands use egress/uninstall.sh.

set +x
set -Eeuo pipefail

readonly ONE_BROWSER_EGRESS_UNINSTALLER_URL='https://raw.githubusercontent.com/voiceofhu/one-action/main/egress/uninstall.sh'
...
egress_compat_local_entrypoint() {
  ...
  entrypoint_dir=$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd) ||
    return 1
  [ -f "$entrypoint_dir/egress/uninstall.sh" ] || return 1
  printf '%s/egress/uninstall.sh' "$entrypoint_dir"
}
...
egress_compat_run() {
  ...
  if [ -z "$entrypoint" ]; then
    command -v curl >/dev/null || {
      printf 'Error: curl is required to load the Egress uninstaller\n' >&2
      exit 1
    }
    ...
    curl -q --proto '=https' --tlsv1.2 \
      ...
      "$ONE_BROWSER_EGRESS_UNINSTALLER_URL" \
